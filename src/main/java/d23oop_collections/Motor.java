...
public interface Motor {
    ...
    Bir interface, bir veya daha fazla abstract method içerir ve bu methodların imzalarını (adlarını ve parametrelerini)
    belirtir, ancak bu methodların nasıl uygulanacağını tanımlamaz.

    Class'lar, bu interface'leri uygulayarak (implement ederek) bu methodları kendi ihtiyaçlarına göre tanımlarlar.*/

    //------------------------------------------------------------------------------------------------------------------

    /*i) interface icindeki method'lar otomatik olarak 'public' dir, bu yuzden interface'lerde method olustururken
    access modifier yazmayiz.(static degillerdir)

    ii) interface icindeki method'lar otomatik olarak 'abstract' dir, bu yuzden interface'lerde abstract method
    olustururken 'abstract' keyword'unu ve 'method body' sini yazmayiz.   */
    ...
    /*4- Coklu interface parentlarin her birinin icine ayni isimli methodlar koyabiliriz. Mesela Motor'un, Klima'nin ve
    Fren'nin iclerine run() methodu koymak gibi. Child class bu methodu override ettiginde bu methodun hangi
    interface'den geldiği belirsizdir.*/
    ...
    /*7) Normalde interface icine concrete (Somut) method konulamaz ama bazi özel durumlarda concrete method koymak gerekirse;
    ...
    ==>  default keywordunu kullanarak olusturdugumuz concrete methodlara interface'in child class'indan object
    olusturarak ulasilabilir*/

    /*8) Abstract Class ile Interface arasindaki farklar nelerdir?

    a) Abstract class'lar hem abstract method hemde concrete methodlar icerebilir.fakat interface'ler varsayilan olarak
    abstract method icerir. Ama interfacelerde istersek default ve static keywordllerini kullanarak concrete method
    olusturabiliriz

    b) Abstract class'lar multiple inheritance'i desteklemez ama interface'ler destekler

    c) Abstract class'lar icinde her turlu variable olusturulabilir,
    interfaceler icindeki variable'lar public, static ve final olmak zorundadir
