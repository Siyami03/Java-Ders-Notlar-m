...
public class Maps01 {
    public static void main(String[] args) {
        ...
        paralel olarak gerçekleştirilebilir. Web tarayıcıları, bu kavramın güzel bir örneğidir;çünkü kullanıcı arayüzünü
        donmadan, aynı anda birden fazla sekmede içerik yükleyebilir ve kullanıcı etkileşimlerine yanıt verebilirler.

        Synchronized:
        Synchronized, bir metodun veya bloğun aynı anda sadece bir iş parçacığı tarafından erişilebilir olmasını
        sağlayan bir anahtar kelimedir. Bu, özellikle paylaşılan kaynaklara erişimde veri tutarlılığını ve bütünlüğünü
        korumak için kullanılır.*/

        //--------------------------------------------------------------------------------------------------------------

        /*HashMap ile HashTable arasindaki fark nedir?
        1)  a) HashMap non-synchronized'dir ve bu yuzden thread-safe degildir, yani birden fazla iş parçacığı aynı anda
        HashMap nesnesine erişebilir. Bu, yüksek performanslı durumlar için avantajlı olabilir ancak çoklu iş parçacığı
        ortamlarında veri tutarlılığını garantilemek için dış senkronizasyon mekanizmaları kullanılmalıdır.
        (Programci Synchronized Bloklar, Lock ve Semapforlar gibi ozel yapilar kullanir.)

            b) HashTable synchronized ve thread-safe'dir (yani aynı anda sadece bir iş parçacığı aynı Hashtable'i
        kullanabilir. kuyruk olusur ve hizmet suresi uzar)

        2) a)HashMap bir tane null key'e (unique olmali, 2 null olamazdi, ustune yazar) ve istediginiz kadar null
        value'a musaade eder
           b)Hashtable, anahtarlar ve değerler için null değerlerine izin vermez. Herhangi bir null değer eklemeye
        çalışırsanız, NullPointerException alırsınız.
        ...
        //ogrenciNotlari.put(null,50);                HATA Hashtable null'ı kabul etmez. Tutarlılık ve performans için
        //ogrenciNotlari.put("Ahmet",null);           HATA bunu da kabul etmez
        ...
        /*1) Treemap'ler entry'leri natural order'a gore siraya koyar, bu yuzden yavastirlar
          ...
          3) Treemap'lerin key'lerinde null kullanilamaz(Comparator istisnasi var ama onerilmez), ama value kisimlarinda
        ...
        System.out.println(countryPoulation);         //{France=null, Germany=83000000, Italy=null, Netherland=18000000,
                                                      //Turkiye=83000000, USA=400000000}

        //--------------------------------------------------------------------------------------------------------------

        /*LinkedHashMap, HashMap'e benzer, ancak ekleme sırasına göre sıralanmış bir şekilde verileri tutar.Yani,
        LinkedHashMap verileri eklediğiniz sırayla tutar ve bu sırayı korur. Bu nedenle, verileri eklediğiniz sırayla
        geri almanız gerektiğinde kullanışlı olabilir.

        **ornegin,bir takvim uygulamasında, etkinlikleri tarih sırasına göre tutmak için kullanılabilir. yada bir
        restoranın sipariş alım sistemi düşünün. Müşterilerin siparişlerini verme sırası, restoranın sipariş hazırlama
        sırasını belirler.

        Aynı mantık, müşteri hizmetleri çağrı merkezleri, sırayla hizmet veren işletmeler veya benzeri senaryolarda da
        kullanılabilir. Müşteri veya işlem sıralarını takip etmek için LinkedHashMap tercih edilebilir.*/
