...
public class Lambda09 {
    ...
    //Comparator.comparing():Comparator sınıfında bulunan comparing metodu, belirtilen bir fonksiyona göre karşılaştırma
    //yapmak için bir karşılaştırıcı (comparator) oluşturur.

    //sorted(Comparator.comparing(t->t.length())) ifadesi, stream içindeki elemanları uzunluklarına göre artan sırayla
    //sıralar. Eğer ters sıralama istenirse, reversed() metodu eklenerek sorted(Comparator.comparing(String::length).
    //reversed()) şeklinde kullanılabilir.
