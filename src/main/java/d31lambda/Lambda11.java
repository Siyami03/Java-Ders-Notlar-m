...
public class Lambda11 {
    ...
    b) range - Belirtilen başlangıç (dahil) ve bitiş değerleri (haric) arasında ardışık sayıları temsil eden bir akış
    oluşturur.
    c) rangeClosed - Belirtilen başlangıç (dahil) ve bitiş değerleri (dahil) arasında ardışık sayıları temsil eden bir
    akış oluşturur.
    ...
    public static int getSumFromSeventoSeventy(){
        return IntStream.rangeClosed(7,70).reduce(Math::addExact).getAsInt();
                          // Math:: addExact (overFlow kontrolu var) yerine Integer class'taki sum()'da kullanılabilirdi
    }
    ...
    public static int getSumOfBetweenTwoInteger(int a,int b){                     //Yer degistirme algoritmasi yapyik
        if (a > b){
            int temp = a;
            a = b;
            b = temp;
